...
function StopCarExam(success)
	if success then
		TriggerServerEvent('esx_bl_carlicense:addLicense', "car_license")
...
function OpenCARSchoolMenu()
	...
	local elements = {}
		...
		if not ownedLicenses['car_license'] then
		...
		if ownedLicenses['car_license'] then
			table.insert(elements, {
				label = ("Ya tienes la licencia de autos, consulta /verlicencia")
			})
		end

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'car_license_actions', {
		title    = "Escuela de conduccion de autos",
		elements = elements,
		align    = 'right'
	}, function(data, menu)
		if data.current.value == 'car_test' then
			menu.close()
			ESX.TriggerServerCallback('esx_bl_carlicense:canYouPay', function(haveMoney)
				if haveMoney then
...
AddEventHandler('esx_bl_carlicense:hasEnteredMarker', function(zone)
	if zone == 'CARSchool' then
		CurrentAction     = 'carlicense_menu'
		CurrentActionMsg  = "Pulse [E] para abrir el menu"
		CurrentActionData = {}
	end
...
AddEventHandler('esx_bl_carlicense:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)

RegisterNetEvent('esx_bl_carlicense:loadLicenses')
AddEventHandler('esx_bl_carlicense:loadLicenses', function(licenses)
	Licenses = licenses
end)

CreateThread(function()
	local blip = AddBlipForCoord(Config.Zones.CARSchool.Pos.x, Config.Zones.CARSchool.Pos.y, Config.Zones.CARSchool.Pos.z)
	...
	AddTextComponentSubstringPlayerName("Escuela de manejo de autos")
...
CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if (IsControlJustReleased(0, 38)) and (CurrentAction == 'carlicense_menu') then
					OpenCARSchoolMenu()
					ESX.HideUI()
					CurrentAction = nil
			end
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_bl_carlicense:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_bl_carlicense:hasExitedMarker', LastZone)
			ESX.HideUI()
		end
		Wait(sleep)
	end
