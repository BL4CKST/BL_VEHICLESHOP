...
function StopAirExam(success)
	if success then
		TriggerServerEvent('esx_bl_airlicense:addLicense', "air_license")
...
function OpenAIRSchoolMenu()
	...
	local elements = {}
		...
		if not ownedLicenses['air_license'] then
		...
		if ownedLicenses['air_license'] then
			table.insert(elements, {
				label = ("Ya tienes la licencia de avion, consulta /verlicencia")
			})
		end

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'air_license_actions', {
		title    = "Escuela de conduccion de aviones",
		elements = elements,
		align    = 'right'
	}, function(data, menu)
		if data.current.value == 'air_test' then
			menu.close()
			ESX.TriggerServerCallback('esx_bl_airlicense:canYouPay', function(haveMoney)
				if haveMoney then
		...
		menu.close()
		CurrentAction     = 'airlicense_menu'
		CurrentActionMsg  = "Pulse [E] para abrir el menu"
		
	end)
end

AddEventHandler('esx_bl_airlicense:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_bl_airlicense:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)

RegisterNetEvent('esx_bl_airlicense:loadLicenses')
AddEventHandler('esx_bl_airlicense:loadLicenses', function(licenses)
	Licenses = licenses
end)
...
CreateThread(function()
	...
	AddTextComponentSubstringPlayerName("Escuela de manejo de aviones")
...
CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if (IsControlJustReleased(0, 38)) and (CurrentAction == 'airlicense_menu') then
					OpenAIRSchoolMenu()
					ESX.HideUI()
					CurrentAction = nil
			end
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_bl_airlicense:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_bl_airlicense:hasExitedMarker', LastZone)
			ESX.HideUI()
		end
		Wait(sleep)
	end
